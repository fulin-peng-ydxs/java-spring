...
public class SimpleRedisTemplateConfig {
    ...
    public JedisConnectionFactory redisConnectionFactory() {
        //旧版配置：1.x
//        //创建jedis连接池来支持连接工厂对连接的创建：配置的简单访问，按需还可以配置哨兵访问、主从和集群访问等
...
//        jedisConnectionFactory.setPort(6379);
        ...
        redisStandaloneConfiguration.setPassword("123");
    ...
    private Jackson2JsonRedisSerializer<Object> jsonSerializer() {
        ...
        ObjectMapper objectMapper = new ObjectMapper();
         //1.指定序列化和反序列化时可借助的方法和能够处理的字段类型：设置所有
        objectMapper.setVisibility(PropertyAccessor.ALL, JsonAutoDetect.Visibility.ANY);
         //2.设置类型信息化存储，用于准确反序列化对象（例如java的多态机制），不需要将final类型的对象做处理,解释如下：
